...
export async function projectDom(page: Page, options?: ProjectionOptions): Promise<ProjectedSnapshot> {
  ...
  const result = await page.evaluate<BrowserProjectionResult, Required<ProjectionOptions>>(
    (opts) => {
      ...
      const attrWhitelist = new Set(['id', 'data-testid', 'name', 'placeholder', 'value', 'aria-label', 'aria-labelledby', 'aria-describedby', 'aria-expanded', 'aria-pressed', 'aria-selected']);
      ...
      const toPath = (pathParts: string[]): string => {
