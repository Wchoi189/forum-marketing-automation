...
### Sprint plan: `scheduler-sov-sprint-1.md` (archived)

Written at `.planning/spec-kit/archive/plans/sprints/scheduler-sov-sprint-1.md`.
Five phases:
...
- [x] SoV thresholds (5% / 20%) — do these match observed board behaviour?
- [x] Combined clamp bounds [0.5, 2.0] — acceptable risk range?
- [x] `OUR_AUTHOR_SUBSTRING` default `'shareplan'` — is this the right substring?
- [x] SoV averaging strategy (mean across window snapshots) — or prefer latest-only?
- [x] No UI changes in Sprint 1 — acceptable to expose `sovFactor` in API only for now?
